...
export default function CreateOrderModal({ isOpen, onClose }: CreateOrderModalProps) {
    ...
    // Calculate total price and check for stock/duplicates
    useEffect(() => {
        let total = 0;
    ...
    const onSubmit = async (data: OrderFormData) => {
        ...
        if (hasDuplicates) {
            return toast.error("Duplicate product entries in the same order.");
        }
        ...
        for (const item of data.products) {
            ...
            if (item.quantity > product.stock) {
                return toast.error(`Only ${product.stock} items available in stock for "${product.name}".`);
            }
            if (product.status === 'Out of Stock' || product.stock <= 0) {
                return toast.error(`Product "${product.name}" is currently unavailable.`);
            }
        }
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[640px] p-0 overflow-hidden rounded-2xl border-none shadow-2xl">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6 bg-white max-h-[70vh] overflow-y-auto">
